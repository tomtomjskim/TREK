...
import {
  AuthAttemptCancelledError,
  beginExternalAuthAttempt,
  isAuthAttemptCancelled,
  useAuthStore,
...
export function useLogin() {
  ...
  useEffect(() => {
    ...
    if (oidcCode) {
      ...
      setIsLoading(true);
      void Promise.resolve(beginExternalAuthAttempt())
        .then(async (attempt) => {
          try {
            if (!attempt.isCurrent()) throw new AuthAttemptCancelledError();
            const response = await fetch('/api/auth/oidc/exchange?code=' + encodeURIComponent(oidcCode), {
              credentials: 'include',
            ...
            if (data.token) {
              if (!(await loadUser()) || !attempt.isCurrent()) return;
            ...
            if (isAuthAttemptCancelled(err) || !attempt.isCurrent()) return;
            window.history.replaceState({}, '', '/login');
        ...
        .catch((err: unknown) => {
          // beginExternalAuthAttempt can fail before an attempt exists when a
          // durable pending logout cannot be confirmed. Handle it here rather
          // than leaving the callback as an unhandled rejection/spinner.
          window.history.replaceState({}, '', '/login');
          setError(getApiErrorMessage(err, t('login.oidcFailed')));
          setIsLoading(false);
        });
      return;
    }
        ...
        if (config) {
          ...
          if (
            ...
            void Promise.resolve(beginExternalAuthAttempt())
              .then((attempt) => {
                try {
                  if (attempt.isCurrent() && !wasSignedOut()) window.location.href = '/api/auth/oidc/login';
                } finally {
                  attempt.finish();
                }
              })
  ...
  const handlePasskeyLogin = async (): Promise<void> => {
    setError('');
    setIsLoading(true);
    let attempt: Awaited<ReturnType<typeof beginExternalAuthAttempt>> | null = null;
    try {
      attempt = await beginExternalAuthAttempt();
      if (!attempt.isCurrent()) throw new AuthAttemptCancelledError();
      const options = await authApi.passkey.loginOptions(attempt.signal);
      if (!attempt.isCurrent()) throw new AuthAttemptCancelledError();
      const assertion = await startAuthentication({ optionsJSON: options });
      if (!attempt.isCurrent()) throw new AuthAttemptCancelledError();
      await authApi.passkey.loginVerify(assertion, attempt.signal);
      if (!attempt.isCurrent()) throw new AuthAttemptCancelledError();
      if (!(await loadUser({ silent: true })) || !attempt.isCurrent()) return;
      takeOff();
    } catch (err: unknown) {
      if (isAuthAttemptCancelled(err) || (attempt && !attempt.isCurrent())) return;
  ...
  const handleOidcLogin = async (event: React.MouseEvent<HTMLAnchorElement>): Promise<void> => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      if (passwordChangeStep) {
        ...
        const attempt = await Promise.resolve(beginExternalAuthAttempt());
        try {
          if (!useAuthStore.getState().isAuthenticated || !attempt.isCurrent()) {
          ...
          if (!attempt.isCurrent()) throw new AuthAttemptCancelledError();
          if (!(await loadUser({ silent: true })) || !attempt.isCurrent()) {
            setIsLoading(false);
            return;
          }
          takeOff();
