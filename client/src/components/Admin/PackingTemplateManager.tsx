...
export default function PackingTemplateManager() {
  ...
  const applyDetailData = (session: DetailSession, data: { categories?: TemplateCategory[]; items?: TemplateItem[] }, syncSummary = false) => {
  ...
  const toggleExpand = async (id: number) => {
    ...
    try {
      ...
      applyDetailData(
        session,
        data,
        canSyncSummary && !pendingActionsRef.current.has(templateMutationKey(id)),
      )
    } catch {
      if (isCurrentDetailSession(session)) {
        toast.error(t('admin.packingTemplates.loadError'))
