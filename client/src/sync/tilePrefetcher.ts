...
async function prefetchTilesWithStatus(
  ...
  async function worker(): Promise<void> {
    while (cursor < coords.length) {
      ...
      // The SW CacheFirst handler stores the response. Opaque no-cors raster
      // responses are valid; other non-ok responses are incomplete and must
      // not be counted as cached tiles.
      try {
...
export async function prefetchTilesForTrip(
  ...
  const result = await prefetchTilesWithStatus(bbox, template, 0, 16, cartoKey, activeLease, activeClearGeneration)
