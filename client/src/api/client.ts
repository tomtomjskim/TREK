import {
  ...
  type PlaceEnrichmentApplyResult,
  type PlaceEnrichmentPreviewRequest,
  type PlaceEnrichmentPreviewResult,
...
export const placesApi = {
  ...
  previewEnrichment: (tripId: number | string, data: PlaceEnrichmentPreviewRequest) =>
    apiClient
      .post(`/trips/${tripId}/places/enrichment/preview`, data)
      .then((r) => r.data as PlaceEnrichmentPreviewResult),
...
export const pluginsApi = {
  ...
  journalEntryRows: (entryId: number) =>
    apiClient
      .get(`/journal-entry-rows/${entryId}`)
      .then(
        (r) =>
          r.data as {
            providers: Array<{ pluginId: string; items: Array<{ label: string; value?: string; url?: string }> }>;
  ...
  myActivity: (limit = 200) =>
    apiClient
      .get(`/plugin-activity?limit=${limit}`)
      .then(
        (r) =>
          r.data as {
            activity: Array<{
...
export const mapsApi = {
  ...
  pois: (
    ...
    apiClient
      .get('/maps/pois', { params: { category, ...bbox, lang }, signal, timeout: 20000 })
      .then(
        (r) =>
          r.data as {
...
export const budgetApi = {
  ...
  createSettlement: (
    tripId: number | string,
    data: { from_user_id: number; to_user_id: number; amount: number; currency?: string }
