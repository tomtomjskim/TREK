...
for (const viewport of viewports) {
  test(`trip map controls avoid both panel toggles at ${viewport.width}px`, async ({ page }) => {
    const tripResponse = await page.request.post('/api/trips', {
      data: { title: `E2E Responsive Map Controls ${viewport.width} ${Date.now()}` },
    });
    ...
    try {
      ...
      // The whole nominal 44px target must sit above the map. When part of the
      // negative-z button overlaps its own panel, that covered strip reports the
      // panel from elementFromPoint even though boundingBox still says 44px.
      const [leftInnerEdgeIsClickable, rightInnerEdgeIsClickable] = await Promise.all([
        leftToggle.evaluate((button) => {
        ...
        rightToggle.evaluate((button) => {
          const rect = button.getBoundingClientRect();
          const hit = document.elementFromPoint(rect.right - 2, rect.top + rect.height / 2);
          return hit === button || Boolean(hit && button.contains(hit));
        }),
      ]);
      expect(leftInnerEdgeIsClickable, 'left toggle keeps its full 44px hit target').toBe(true);
      expect(rightInnerEdgeIsClickable, 'right toggle keeps its full 44px hit target').toBe(true);
