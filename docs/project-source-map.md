...
> 마지막 확인: 2026-07-30, v3.4.1 포크의 Vacay R0.3a 로컬 작업 기준
>
...
| Vacay destructive year/invite flow | `server/src/services/vacayService.ts`, Nest Vacay controller/service, MCP Vacay tools, `client/src/pages/VacayPage.tsx`와 `pages/vacay/useVacay.ts` | 연도 삭제와 초대 수락을 immediate transaction으로 처리; 누락된 이관 연도·다중 accepted·owner orphan·fused/pending/ambiguous ownership은 fail-closed하고 canonical positive ID만 허용하며 실제 상태 변경 뒤에만 best-effort 알림 |
