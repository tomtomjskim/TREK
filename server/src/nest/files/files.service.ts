...
export class FilesService {
  ...
  authenticateDownload(req: Request): { userId: number } | { error: string; status: number } {
    ...
    if (queryToken) {
      const consumed = this.tokens.consumeWithMeta(queryToken, 'download');
      if (!consumed) return { error: 'Invalid or expired token', status: 401 };
      if (consumed.sid && isSessionIdRevoked(this.db.connection, consumed.sid)) {
        return { error: 'Invalid or expired token', status: 401 };
      }
      const row = this.db.get<{ password_version?: number }>(
