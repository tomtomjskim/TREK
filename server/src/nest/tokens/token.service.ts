...
export class TokenService {
  ...
  createWsToken(userId: number, binding: SessionBinding): { error?: string; status?: number; token?: string } {
    // Carry the exact source JWT binding validated by the guard. Re-reading the
    // current DB version here would let an old request get upgraded across a
    // concurrent password reset and mint a valid capability from a stale JWT.
    const token = this.ephemeral.create(userId, 'ws', binding);
  ...
  createResourceToken(
    ...
    const token = this.ephemeral.create(userId, purpose, binding);
    if (!token) return { error: 'Service unavailable', status: 503 };
    return { token };
  }
