...
export class AutoBackupJob implements OnApplicationBootstrap {
  ...
  async runBackup(): Promise<void> {
    // Load retention before the create attempt so cleanup also runs when the
    // archive cannot be created (for example, ENOSPC). Reclaiming old archives
    // is what lets a later scheduled tick recover from a full disk.
    const settings = loadSettings();
    try {
