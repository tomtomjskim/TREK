...
export function createPluginRpcHostFactory(dbs: DatabaseService): PluginRpcHostFactory {
  ...
  const places = new PlacesService(dbs, permissions, realtime, new MapsService(dbs, photoCache, meteredGoogleApiTransport(dbs)), queryHelpers, unsplash, photoCache, journey, generalStorage);
  ...
  const trips = new TripsService(dbs, reservations, days, permissions, budget, realtime, unsplash, generalStorage, addons);
