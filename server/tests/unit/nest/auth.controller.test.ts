...
function sessionReq(userId = 1, pv = 0, sid = 'browser-session'): Request {
  const payload = Buffer.from(JSON.stringify({ id: userId, pv, sid })).toString('base64url');
  return { ip: '9.9.9.9', headers: {}, cookies: { trek_session: `e30.${payload}.sig` } } as unknown as Request;
}
...
describe('AuthPublicController', () => {
  ...
  it('mfa/verify-login sets cookie + audits; logout clears cookie', () => {
    ...
    const logoutRequest = { ...req, cookies: { trek_session: 'session.jwt' } } as unknown as Request;
    expect(
      apc(asvc({ clearAuthCookie, revokeSessionToken } as Partial<AuthService>), rl()).logout(logoutRequest, res),
    ).toEqual({ success: true });
    expect(revokeSessionToken).toHaveBeenCalledWith('session.jwt');
    expect(clearAuthCookie).toHaveBeenCalledWith(res, logoutRequest);
    expect(revokeSessionToken.mock.invocationCallOrder[0]).toBeLessThan(clearAuthCookie.mock.invocationCallOrder[0]);
  });

  it('AUTH-LOGOUT-REVOCATION-001: clears the browser cookie even when revocation storage fails', () => {
    const clearAuthCookie = vi.fn();
    const revokeSessionToken = vi.fn(() => {
  ...
  it('AUTH-LOGOUT-REVOCATION-002: revokes distinct cookie and bearer sessions', () => {
    const clearAuthCookie = vi.fn();
    const revokeSessionToken = vi.fn();
    const logoutRequest = {
      ...req,
      cookies: { trek_session: 'cookie.jwt' },
    ...
    expect(revokeSessionToken).toHaveBeenNthCalledWith(1, 'cookie.jwt');
    expect(revokeSessionToken).toHaveBeenNthCalledWith(2, 'bearer.jwt');
    expect(clearAuthCookie).toHaveBeenCalledWith(res, logoutRequest);
  });

  it('AUTH-LOGOUT-REVOCATION-003: tries the second token and rethrows the first failure', () => {
    const clearAuthCookie = vi.fn();
    const revokeSessionToken = vi.fn((token: string | undefined) => {
      if (token === 'cookie.jwt') throw new Error('cookie revoke failed');
      return null;
    });
    const logoutRequest = {
      ...req,
    ...
    expect(() =>
      apc(asvc({ clearAuthCookie, revokeSessionToken } as Partial<AuthService>), rl()).logout(logoutRequest, res),
    ).toThrow('cookie revoke failed');
    expect(revokeSessionToken).toHaveBeenCalledTimes(2);
    expect(revokeSessionToken).toHaveBeenLastCalledWith('bearer.jwt');
    expect(clearAuthCookie).toHaveBeenCalledWith(res, logoutRequest);
  });

  it('AUTH-LOGOUT-REVOCATION-004: revokes a duplicate raw token only once and preserves invalid-token handling', () => {
    const clearAuthCookie = vi.fn();
    const revokeSessionToken = vi.fn(() => null);
    const logoutRequest = {
      ...req,
      cookies: { trek_session: 'not-a-jwt' },
    ...
    expect(revokeSessionToken).toHaveBeenCalledWith('not-a-jwt');
  });
});
...
describe('AuthController (authenticated)', () => {
  ...
  it('resource-token 503 when unavailable, else returns the token payload', () => {
    expect(
      thrown(() =>
        ac(asvc({}), rl(), { createResourceToken: vi.fn().mockReturnValue(null) }).resourceToken(
          user,
          {},
          sessionReq(),
        ),
    ...
    expect(
      ac(asvc({}), rl(), { createResourceToken: vi.fn().mockReturnValue({ token: 'rt' }) }).resourceToken(
        user,
        { purpose: 'download' },
        sessionReq(),
      ),
  ...
  it('ws/resource tokens throttle on their own buckets, so a mint loop cannot drain the store or block login', () => {
    ...
    expect(
      thrown(() =>
        ac(asvc({}), s, { createWsToken: vi.fn().mockReturnValue({ token: 'ws' }) }).wsToken(user, sessionReq()),
      ),
    ...
    expect(
      thrown(() =>
        ac(asvc({}), s2, { createResourceToken: vi.fn().mockReturnValue({ token: 'rt' }) }).resourceToken(
          user,
          {},
          sessionReq(),
        ),
  ...
  it('the ws/resource ceilings are per account, not per address: one heavy user cannot 429 the office', () => {
    ...
    expect(
      ac(asvc({}), s, { createWsToken: vi.fn().mockReturnValue({ token: 'ws2' }) }).wsToken(other, sessionReq(2)),
    ).toEqual({ token: 'ws2' });
    expect(
      ac(asvc({}), s, { createResourceToken: vi.fn().mockReturnValue({ token: 'rt2' }) }).resourceToken(
        other,
        {},
        sessionReq(2),
      ),
  ...
  it('change-password carries the session cookie remember claim into the service and the re-issued cookie (#1927)', () => {
    ...
    const remembered = jwt.sign({ id: 1, pv: 0, remember: true, sid: 'password-session' }, 'any-secret');
    ...
    expect(changePassword).toHaveBeenCalledWith(1, 'u@example.test', anyBody(), true, 'password-session');
  ...
  it('ws-token maps error, else returns the token', () => {
    expect(
      thrown(() =>
        ac(asvc({}), rl(), { createWsToken: vi.fn().mockReturnValue({ error: 'down', status: 503 }) }).wsToken(
          user,
          sessionReq(),
        ),
      ),
    ...
    expect(
      ac(asvc({}), rl(), { createWsToken: vi.fn().mockReturnValue({ token: 'ws' }) }).wsToken(user, sessionReq()),
    ).toEqual({ token: 'ws' });
  });
