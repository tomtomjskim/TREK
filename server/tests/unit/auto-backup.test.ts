...
const dbMock = vi.hoisted(() => ({
  ...
  closeDb: vi.fn(),
  reinitialize: vi.fn(),
  getDatabaseFilePath: vi.fn(() => path.join(__dirname, '../../data', 'travel.db')),
}));
...
describe('auto-backup run', () => {
  ...
  it('keeps the auto-backup-*.zip naming so retention still prunes it', async () => {
    ...
    expect(logMock.logInfo).toHaveBeenCalledWith(
      expect.stringMatching(/^Auto-Backup created: auto-backup-[\w-]+\.zip$/),
    );
  ...
  it('logs the failure, drops the partial zip and still runs retention', async () => {
    fsMock.existsSync.mockImplementation(
    ...
    // Nothing was committed (put commits by rename only on success), while the
    // half-built zip spool and snapshot scratch are cleaned up. Retention still
    // runs so a disk-full failure can recover space on the next tick.
    ...
    expect(storageStub.list).toHaveBeenCalledWith('backups');
    expect(storageStub.delete).toHaveBeenCalledWith('backups', 'auto-backup-2020-01-01T02-00-00.zip');
  });
});
